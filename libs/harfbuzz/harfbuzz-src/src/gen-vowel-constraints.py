	...
	def __str__(self, index=0, depth=4):
		...
		if isinstance (self._c, list):
			if len (self._c) == 0:
				...
				s.extend(
					(
						'{}if (0x{:04X}u == buffer->cur ({}).codepoint &&\n'.format(
							indent, self._c[0], index
						),
						f'{self._indent(depth + 2)}buffer->idx + {len(self._c)} < count &&\n',
					)
				)
				s.extend(
				...
				s.append(f'{indent}{{\n')
				s.extend(
					f'{self._indent(depth + 1)}buffer->next_glyph ();\n'
					for _ in range(len(self._c))
				)
				s.extend(
					(
						f'{self._indent(depth + 1)}_output_dotted_circle (buffer);\n',
						f'{indent}}}\n',
					)
				)
		else:
...
with io.open (sys.argv[1], encoding='utf-8') as f:
	constraints_header = [f.readline ().strip () for _ in range (2)]
	for line in f:
		j = line.find ('#')
		if j >= 0:
			line = line[:j]
...
print (' *')
print(f' *   {sys.argv[0]} use Scripts.txt')
print (' *')
